...
# 将图片的 color model 转化成 RGB model
def cvtColor(image):
...
class DataGenerator(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = self.getRandomData(image, self.input_shape, random=self.random)
        image = np.transpose(preprocessInput(np.array(image).astype(np.float32)), [2, 0, 1])
    ...
    def getRandomData(self, image, inpt_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
